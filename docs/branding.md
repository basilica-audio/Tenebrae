...
Tenebrae is part of the **Basilica Audio** plugin suite. Two names are in play, and they are not
...
| Audio Unit `name` string | **Basilica Audio: Tenebrae** | derived from `COMPANY_NAME` + `PRODUCT_NAME` |
...
| User preset folder (macOS) | `~/Library/Audio/Presets/Basilica Audio/Tenebrae/` | `PresetManagerConfig::manufacturerName` |
| User preset folder (Windows) | `%APPDATA%\Basilica Audio\Tenebrae\Presets\` | same |

## What stays on the old name, on purpose

| Identifier | Value | Why it does not move |
| --- | --- | --- |
| `BUNDLE_ID` | `com.yvesvogl.tenebrae` | A host recognises a plugin across sessions by its bundle ID. Changing it makes every existing project treat this as a *different* plugin and lose its instance. Nothing a user sees flows from it, so there is no benefit to weigh against that. |
| Preset JSON `plugin` field | `com.yvesvogl.tenebrae` | The same string. Freezing it keeps every preset file ever written importable, including ones exported by older builds and shared between users. |
...
Presets used to live under `~/Library/Audio/Presets/Yves Vogl/Tenebrae/` (macOS) and
`%APPDATA%\Yves Vogl\Tenebrae\Presets\` (Windows). Nothing there is lost:

- On the first launch after updating, `PresetManager` **copies** every `.basilicapreset` file from
  the old folder into the new one.
- It **never moves or deletes** the originals, so an older build of Tenebrae - or a downgrade -
